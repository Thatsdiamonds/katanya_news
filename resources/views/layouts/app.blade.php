    ...
    <body class="font-sans antialiased text-gray-900 bg-[#F9FAFB] flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-white border-r border-gray-200 flex flex-col hidden md:flex z-10 shrink-0">
            <div class="h-16 flex items-center px-6 border-b border-gray-100">
                <a href="{{ route('home') }}" class="font-serif text-xl font-bold tracking-tight text-gray-900 flex items-center gap-1.5">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                <ul role="list" class="flex flex-1 flex-col gap-y-1">
                    <li>
                        <a href="{{ route('admin.dashboard') }}" class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('admin.dashboard') ? 'bg-gray-50 text-blue-600' : 'text-gray-700 hover:text-blue-600 hover:bg-gray-50' }}">
                    ...
                    <li>
                        <a href="{{ route('admin.news.index') }}" class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('admin.news.*') ? 'bg-gray-50 text-blue-600' : 'text-gray-700 hover:text-blue-600 hover:bg-gray-50' }}">
                            <svg class="h-6 w-6 shrink-0 {{ request()->routeIs('admin.news.*') ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-600' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    <li>
                        <a href="{{ route('admin.categories.index') }}" class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('admin.categories.*') ? 'bg-gray-50 text-blue-600' : 'text-gray-700 hover:text-blue-600 hover:bg-gray-50' }}">
                    ...
                    <li>
                        <a href="{{ route('admin.users.index') }}" class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('admin.users.*') ? 'bg-gray-50 text-blue-600' : 'text-gray-700 hover:text-blue-600 hover:bg-gray-50' }}">
            ...
            <div class="p-4 border-t border-gray-100 mt-auto">
                <div class="flex items-center px-2 mb-4">
                    <div class="h-8 w-8 rounded-full bg-gray-100 ring-1 ring-gray-200 flex items-center justify-center font-bold text-gray-600 text-xs uppercase overflow-hidden">
                        @if(auth()->user()->avatar)
                            <img src="{{ Storage::disk('public')->url(auth()->user()->avatar) }}" alt="{{ auth()->user()->name }}" class="h-full w-full object-cover">
                        @else
                    ...
                    <div class="ml-3 truncate">
                        ...
                        <p class="text-xs font-medium text-gray-500 capitalize truncate">{{ auth()->user()->role }}</p>
                    </div>
                </div>
                <form method="POST" action="{{ route('logout') }}" onsubmit="return window.confirm('Apakah Anda yakin ingin logout?');">
                    @csrf
